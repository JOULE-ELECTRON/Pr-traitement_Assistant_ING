...
class JFC2SourceReader:
    def __init__(self, chemin_source):
        self.wb = openpyxl.load_workbook(chemin_source, data_only=True)
        self.ws = self.wb.active

    def get_dates(self):
        # Les dates sont sur la ligne 3, à partir de la colonne 3
        return [self.ws.cell(row=3, column=col).value for col in range(3, self.ws.max_column + 1)]

    def get_production(self):
        # Production > ACP 54NCl sur la ligne 6, à partir de la colonne 3
        return [self.ws.cell(row=6, column=col).value for col in range(3, self.ws.max_column + 1)]

    def get_arrets(self):
        # Arrêts sur les lignes 46 à 51, à partir de la colonne 3
        return {
            'planifies_process': [self.ws.cell(row=46, column=col).value for col in range(3, self.ws.max_column + 1)],
            'planifies_maintenance': [self.ws.cell(row=47, column=col).value for col in range(3, self.ws.max_column + 1)],
            'grande_revision': [self.ws.cell(row=48, column=col).value for col in range(3, self.ws.max_column + 1)],
            'non_planifies_process': [self.ws.cell(row=49, column=col).value for col in range(3, self.ws.max_column + 1)],
            'non_planifies_maintenance': [self.ws.cell(row=50, column=col).value for col in range(3, self.ws.max_column + 1)],
            'externes': [self.ws.cell(row=51, column=col).value for col in range(3, self.ws.max_column + 1)],
        }

    def get_titres(self):
        # Titres sur les lignes 32 (ACP 29) et 33 (ACP 54), à partir de la colonne 3
        return {
            'acp29': [self.ws.cell(row=32, column=col).value for col in range(3, self.ws.max_column + 1)],
            'acp54': [self.ws.cell(row=33, column=col).value for col in range(3, self.ws.max_column + 1)],
        }
...
def traiter_jfc2(chemin_source, chemin_cible, chemin_sortie):
    ...
    writer = JFC2CibleWriter(chemin_cible)
...
if __name__ == "__main__":
    fichier_source = "Fichier Source JFC2.xlsx"
    fichier_cible = "Fichier Cible Jorf.xlsx"
    fichier_sortie = "Fichier Prétraité JFC2.xlsx"
    traiter_jfc2(fichier_source, fichier_cible, fichier_sortie)
